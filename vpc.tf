locals {
  public_subnets = {
    for key, config in var.subnet_config : key => config
    if lookup(config, "public", true)

  }
  private_subnets = {
    for key, config in var.subnet_config : key => config
    if !lookup(config, "public", false)
  }
}
  ...
  lifecycle {
    precondition {
      ...
      error_message = <<-EOT
                      ...
                      AWS Region "${data.aws_availability_zones.available.id}" supports following availability zones:
      ...
      # az :        ${each.value.az}
      # all_az:       ${join(",", data.aws_availability_zones.available.names)}
      # EOT
    }
  }
}
